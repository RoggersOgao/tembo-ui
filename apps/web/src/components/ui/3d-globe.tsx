...
function Marker({
  ...
  return (
    <group ref={groupRef} visible={isVisible}>
      ...
      {/* Circular image at the top — no distanceFactor to prevent blur */}
      <group ref={imageGroupRef} position={topPosition}>
        <Html
          ...
          style={{
            ...
            // Prevent the Html wrapper from inheriting any CSS transforms
            // that could trigger sub-pixel rendering / blur
            willChange: "auto",
        ...
        >
          <div
            ...
            style={{
              width: "32px",
              height: "32px",
              // Force GPU-composited layer at whole-pixel boundaries
          ...
          >
            <img
              ...
              style={{
                ...
                objectFit: "cover",
                imageRendering: "auto",
                // Prevent browser from applying any blur during compositing
...
function Scene({ markers, config, onMarkerClick, onMarkerHover }: SceneProps) {
  ...
  return (
    <>
      {/* Lighting — realistic daylight Earth */}

      {/* Soft ambient fill: just enough to lift the dark side out of pure black,
          simulating atmospheric/earthshine scatter. Kept low so the sun side
          reads as distinctly brighter. */}
      <ambientLight intensity={0.18} color="#c8d8f0" />

      {/* Primary "sun" — bright, slightly warm white, positioned upper-right
          front so the lit hemisphere faces the viewer naturally. High intensity
          drives vivid texture colors without washing them out. */}
      <directionalLight
      ...
      {/* Subtle sky-blue fill from the opposite side — mimics light scattered
          by the atmosphere on the terminator edge, softening the day/night
          boundary without eliminating it. */}
      <directionalLight
      ...
      {/* Faint warm top light — adds a touch of brightness to polar regions
          and prevents the top hemisphere from feeling flat. */}
      <directionalLight
        position={[0, config.radius * 4, config.radius]}
        intensity={0.15}
        color="#ffe8d0"
      />
